...
For each benchmark of a divide-and-conquer SArrow, we will generate the code for a range of sizes and a different number of recursion unrollings representing the level of the algorithm. We will record the execution time on normal laptop as well as high performance computer.
...
\begin{enumerate}
    ...
    \item \textbf{Int count. } It counts the number of occurrences of integer given a list of integer ranging from 0 to 50. \hask{split} divides the list by halves. \hask{count} will count the occurrence of integers and output a list of tuple where the integer is the value and the second integer is the number of occurrences of that value. \hask{union} will union the resulting of sub-lists by summing up the occurrence.
\end{enumerate}
