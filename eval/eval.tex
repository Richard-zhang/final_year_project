...
For merge sort, the speedup increases as the size increases which is shown by the increasing graph from \figref{eval:fig:ms:speed}. However, this trend holds for up to size 26. When the size is 26, we witness a sudden decrease in speedup for various levels. The reason for this abnormal behavior is yet to be studied. We have overproduced the number of threads and witness that overproduction of threads has positive impacts on the speedup. The best level is 7 which contains 128 threads. It gives a 7.5X speedup for merge sort and 9X speedup for dot product when the size is $2^{25}$. Usually, greater number of threads has better speedup for greater sizes. The level that gives the best performance depends on the number of cores. It is recommended to overproduce the number threads compared the number of cores. However, the performance will not be increased greatly and even slower if the level is too big (see the comparison of speedup line for level = 7 and 8).