...
In addition to the generated code, we also show the inferred session types of each role in the system for the merge sort at level two in \coref{eval:code:stypes}. We can call the function \hask{runType} on any SArrow expression to get a list of session types. The session types is pretty printed. The original representation of session types is in free-monad as explained in \charef{chap:spar}. \hask{br} is a syntax sugar for a series of select (see in \secref{spar:sec:session-typing}).
