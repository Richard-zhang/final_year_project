...
For illustration, we use inference rules to explain our proposed strategy for optimized role allocations when composing SArrows. Please see \tabref{SArrow:ra:example} as an example. $x \Rightarrow x$ means the computation starts with role $x$ and ends with role $x$.
